...
## 2026-09-10 - High-Frequency Duration Parsing Optimization with Bounded In-Memory Caching
**Learning:** Parsing workout and timer duration strings (e.g., "1:30", "2m 30s", "45", "60s") inside hot loops, rendering templates, or high-frequency update loops (like WorkoutSession state modifications) triggers costly repeated `.toLowerCase()`, `.trim()`, regex matching, string-splitting, and array indexing. Bypassing these bottlenecks with a module-level in-memory bounded `Map` cache (`durationCache`) keying duration strings to parsed integer seconds achieves massive speed improvements (~7.1x) while keeping heap allocations to zero. Adding a cache-eviction limit (1,000 items) guarantees maximum safety, and exporting a clean invalidation helper (`clearDurationCache()`) ensures perfect test isolation under vitest/jsdom environment runs.
**Action:** Always cache low-level string parsing utility outputs using size-bounded Maps when they are evaluated on render-hot paths or interactive sliders/timers. Provide clear cache invalidation methods for test runners to keep environments completely side-effect free.
