## 2026-09-07 - Hoisting Helpers and Allocation-Free Best Set Loops
**Learning:** Redeclaring inline helper functions (like `getRowData` or `renderIcon` inside `StatsDetailPopup.tsx`) on every render cycle causes high garbage collection pressure and CPU overhead, especially inside multi-tab popups. Hoisting them out of the React body is a zero-cost, high-yield pattern. Similarly, replacing higher-order array methods (like `.reduce()`) inside list iterators (like `.map()` in `FeedLogItem.tsx`) with standard `for` loops completely bypasses closure allocations and function call overhead, making rendering scrolling feeds much smoother.
**Action:** Always lift pure layout helper functions to the module level, and use standard loops instead of functional programming combinators on hot list-rendering paths to reduce memory allocation rates and CPU overhead.
