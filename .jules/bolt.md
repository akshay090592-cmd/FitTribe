## 2026-09-12 - Resilient Micro-Benchmark Assertions for Allocation-Free Loop Refactoring
**Learning:** Micro-benchmarking lightweight numeric calculations (e.g. `calculateWorkoutVolume` over small exercise arrays) in simulated environments like Vitest/Node.js can experience transient V8 JIT timing jitter or garbage collection pauses. Strict `toBeLessThan` assertions on sub-millisecond execution times can result in flaky test runs.
**Action:** When benchmarking small, allocation-free `for` loop refactorings against native `.reduce()`, run high iteration counts (>50,000) and allow a small tolerance margin in test assertions to ensure test stability while eliminating closure allocations.
