## 2026-09-13 - Allocation-Free Workout Volume Calculations in Gamification Logic
**Learning:** Calculating total workout volume (summing `set.weight * set.reps` for completed sets across exercises) using nested `.reduce()` calls (`log.exercises.reduce((acc, ex) => acc + ex.sets.reduce(...))`) inside achievement checks and gamification handlers (`checkAchievements`, `rebuildGamificationState`, `revertGamificationForLog`) creates high CPU overhead and triggers garbage collection churn due to closure function allocations on every exercise and set. Extracting volume calculation into a dedicated `calculateWorkoutVolume` helper using standard index-based nested `for` loops eliminates lambda allocations entirely while providing defensive checks against missing exercise/set properties.
**Action:** Replace nested `.reduce()` calls on exercise and set collections with dedicated, allocation-free, index-based `for` loops to eliminate closure allocations and GC pressure on hot gamification paths.
