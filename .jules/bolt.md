## 2026-09-10 - Bounded Map Caching for Duration String Parsing
**Learning:** High-frequency duration parsing from workout templates and custom plans can lead to hot paths where regular expressions (`/^\d+\s*s?$/`) are repeatedly tested and string splits (`durationStr.split(':')`) are repeatedly executed on identical inputs. Wrapping these parsing steps with a size-bounded memory cache Map (`durationCache`) completely bypasses CPU-heavy string manipulation and regex engine matching, achieving a demonstrated ~8.1x speedup. To maintain clean state transitions across tests in shared execution environments, it is essential to export a `clearDurationCache()` utility and hook it in `beforeEach` setups.
**Action:** Always cache deterministic string-to-numeric parsing routines on hot rendering/interactive paths, and pair them with explicit cache-clearing mechanisms for test isolation.
