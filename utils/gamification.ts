...
export const calculateXP = (logs: WorkoutLog[], options: { isSortedDesc?: boolean } = {}) => {
  ...
  let currentStreak = 0;
  let lastLogMidnight: number | null = null;
  ...
  for (let i = 0; i < len; i++) {
    ...
    if (isStreakEligible) {
      ...
      d.setHours(0, 0, 0, 0);
      const logMidnight = d.getTime();

      if (lastLogMidnight === null) {
        currentStreak = 1;
      } else {
        const diffTime = Math.abs(logMidnight - lastLogMidnight);
...
export const calculateStreaks = (logs: WorkoutLog[], optionsOrReturnLogs: boolean | { returnLogs?: boolean, isSorted?: boolean } = false): number | WorkoutLog[] => {
  ...
  let prevValidMidnight: number | null = null;
  ...
  for (let i = 0; i < logsToProcess.length; i++) {
    ...
    if (prevValidMidnight === null) {
      // First valid log found
      const diffFromToday = Math.round((todayTime - logMidnight) / MS_PER_DAY);
      if (diffFromToday > 3) break; // Last workout too old, streak is 0

      streak = 1;
      prevValidMidnight = logMidnight;
      if (returnLogs) streakLogs.push(log);
    } else {
      const gap = Math.round((prevValidMidnight - logMidnight) / MS_PER_DAY);
 ...
 * Accepts an optional preCalculatedStreak to avoid redundant passes over logs.
 */
